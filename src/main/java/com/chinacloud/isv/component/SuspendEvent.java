...
public class SuspendEvent {
	private static final Logger logger = LogManager.getLogger(SuspendEvent.class);
	@Autowired
	...
	public String go(ValueProvider valueProvider) {
		...
		String revokeResult = oracleUserService.revokePrivileges(statement, valueProvider);
		if(null == revokeResult){
			...
			whiteholeMsg = WhiteholeFactory.getFailedMsg(valueProvider, "处理失败，错误信息： "+revokeResult);
		}
